...
// used when we append our transformed fragments to it. See this AndroidTest commit message for details:
...
const processResponseStatus = response => {
  if (response.status === 200) { // can use status 0 if loading local __files
...
// Required by AndroidTest API 16 AOSP Nexus S emulator.
...
/**
 * Display fetched exist more pages.
...
/**
 * Makes document fragment for a exist more page.
...
var CLASS$2 = { ANDROID: 'pagelib_platform_android', IOS: 'pagelib_platform_ios'
  ...
  /**
   * @param {!Window} window
   * @return {!boolean} true if the user agent is AndroidTest, false otherwise.
   */
